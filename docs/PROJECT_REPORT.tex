...
We quantify P-wave and T-wave preservation through:
\begin{itemize}
    \item \textbf{Amplitude Correlation:} Pearson correlation between ground truth and reconstructed wave amplitudes across beats.
    \item \textbf{Heart Rate Consistency:} Mean absolute error in heart rate estimation between original and reconstructed ECGs.
\end{itemize}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Feature} & \textbf{Ground Truth} & \textbf{Reconstructed} & \textbf{Error} & \textbf{Threshold} \\
\midrule
QRS Duration (ms) & 98.5 $\pm$ 12.3 & 102.1 $\pm$ 14.7 & 6.8 & $<$10 $\checkmark$ \\
PR Interval (ms) & 162.4 $\pm$ 22.1 & 158.9 $\pm$ 24.8 & 9.3 & $<$20 $\checkmark$ \\
QT Interval (ms) & 398.2 $\pm$ 32.6 & 385.4 $\pm$ 38.2 & 18.7 & $<$30 $\checkmark$ \\
Heart Rate (bpm) & 71.8 $\pm$ 14.2 & 72.3 $\pm$ 14.8 & 1.9 & $<$5 $\checkmark$ \\
\midrule
P-wave Amp. Corr. & \multicolumn{2}{c}{$r = 0.78$} & --- & $>$0.7 $\checkmark$ \\
T-wave Amp. Corr. & \multicolumn{2}{c}{$r = 0.84$} & --- & $>$0.7 $\checkmark$ \\
\bottomrule
\end{tabular}
\end{table}

All clinical feature metrics meet established acceptability thresholds. The QRS duration error of 6.8 ms is well within the 10 ms clinical tolerance, ensuring accurate detection of bundle branch blocks. The PR interval error of 9.3 ms preserves atrioventricular conduction assessment capability. The QT interval, critical for arrhythmia risk stratification, shows 18.7 ms error---within the 30 ms threshold but representing the largest relative error, consistent with T-wave reconstruction being more challenging than QRS. Heart rate estimation achieves excellent agreement (1.9 bpm error), confirming R-peak timing preservation.

Figure~\ref{fig:clinical_features} provides comprehensive visualization of these feature-level metrics.

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/clinical_features_evaluation.png}
\caption{ECGGenEval-style clinical feature evaluation. (a) ECG interval measurements comparing ground truth vs. reconstructed signals. (b) Heart rate preservation showing strong correlation ($r > 0.95$). (c) P-wave and T-wave amplitude preservation. (d) Error distribution relative to clinical thresholds (green = pass). (e) Summary confirming all metrics meet clinical acceptability criteria.}
