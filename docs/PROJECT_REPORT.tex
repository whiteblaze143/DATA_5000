...
Following the ECGGenEval framework~\cite{ref63}, we evaluated preservation of clinically relevant features extracted from Lead II. Table~\ref{tab:clinical_features} summarizes the results.

\begin{table}[h]
\caption{Clinical Feature Preservation (Lead II)}
\label{tab:clinical_features}
\begin{tabular}{lccc}
\toprule
\textbf{Feature} & \textbf{MAE} & \textbf{Threshold} & \textbf{Status} \\
\midrule
QRS Duration & 6.8 ms & $<$ 10 ms & \checkmark \\
PR Interval & 9.3 ms & $<$ 20 ms & \checkmark \\
QT Interval & 18.7 ms & $<$ 30 ms & \checkmark \\
Heart Rate & 1.92 bpm & $<$ 5 bpm & \checkmark \\
\bottomrule
\end{tabular}
\end{table}
