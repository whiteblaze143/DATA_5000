...
Kernel Size & 3 \\
Dropout Rate & 0.2 \\
Total Parameters & 17.1 million (shared decoder) \\
 & 40.8 million (lead-specific) \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Optimizer & AdamW \\
Learning Rate & $1 \times 10^{-4}$ (tuned via sweep) \\
Batch Size & 64 \\
Epochs & 150 \\
Loss Function & MSE \\
...
The loss function is mean squared error between predicted and ground truth chest leads:
\begin{equation}
    \mathcal{L} = \frac{1}{5}\sum_{k \in \{V1,V2,V3,V5,V6\}} \text{MSE}(\hat{y}_k, y_k)
\end{equation}
...
We compared two decoder architectures:
...
\textbf{Key Finding:} The simpler shared decoder outperformed the lead-specific decoder on 4 of 5 chest leads, despite having 2.4$\times$ fewer parameters. This suggests that when input information is limited, parameter sharing provides beneficial regularization.
...
\begin{itemize}
    ...
    \item \textbf{Counter-intuitive finding:} Shared decoders outperform lead-specific decoders by 5.3\%
\end{itemize}
