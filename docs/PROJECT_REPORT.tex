...
We conducted an ablation study comparing shared decoder architecture against lead-specific decoders (separate decoder per chest lead). Results from earlier experiments revealed a counter-intuitive finding:

\begin{table}[h]
\caption{Decoder Architecture Ablation}
\label{tab:ablation}
\begin{tabular}{lccc}
\toprule
Architecture & Parameters & DL Leads $r$ & Winner \\
\midrule
Shared Decoder & 17.1M & \textbf{0.846} & \checkmark \\
Lead-Specific (5$\times$) & 40.8M & 0.707 & \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item Bootstrap 95\% CI: $[0.10, 0.18]$ (excludes zero)
    \item Paired $t$-test: $p < 0.001$
\end{itemize}

\textbf{Interpretation:} When input information is limited (only 3 leads), parameter sharing provides beneficial regularization. The lead-specific architecture overfits to training data despite having 2.4$\times$ more parameters.
