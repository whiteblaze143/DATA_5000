...
We also implemented additional safeguards for robust pairwise testing: (1) a minimum paired-sample threshold ("min\_n\_valid", default \(=30\)) to avoid low-powered comparisons; (2) a paired permutation test (sign-flip on paired differences) as a fallback when parametric (t-test) or exact Wilcoxon methods fail due to ties or zero variance; and (3) Benjamini-Hochberg (FDR) correction across leads for each pairwise variant comparison (baseline vs other) to control false discovery rates across multiple tests. These steps help ensure reported p-values are conservative and reproducible.
...
Following the ECGGenEval framework~\cite{ref63}, we evaluated preservation of clinically relevant features extracted from Lead II. Table~\ref{tab:clinical_features_lead2} summarizes the results.

\begin{table}[h]
\caption{Clinical Feature Preservation (Lead II) — Reference Lead}
\label{tab:clinical_features_lead2}
\begin{tabular}{lccc}
    oprule
    extbf{Feature} & \textbf{MAE} & \textbf{Threshold} & \textbf{Status} \\
\midrule
QRS Duration & 2.0 ms & $<$ 10 ms & \checkmark \\
PR Interval & 4.5 ms & $<$ 20 ms & \checkmark \\
QT Interval & 8.6 ms & $<$ 30 ms & \checkmark \\
Heart Rate & 0.12 bpm & $<$ 5 bpm & \checkmark \\
\bottomrule
\end{tabular}
\end{table}
...
Table\textcolor{blue}{~\ref{tab:variant_comparison}} compares per-lead performance across the three architectural variants. We performed pairwise comparisons (baseline vs hybrid, baseline vs physics) using paired $t$-tests, Wilcoxon signed-rank tests, and a paired permutation fallback. Results were adjusted using Benjamini--Hochberg FDR correction and saved to \texttt{\detokenize{results/eval/variant_pairwise_comparisons.csv}}. Leads with insufficient paired samples are listed in \texttt{\detokenize{results/eval/variant_pairwise_skipped.csv}} for auditability.
...
Pairwise comparisons show no statistically significant differences between variants after FDR correction. For example, baseline vs hybrid in V6 (mean difference in QRS duration error = +0.44 ms; baseline minus hybrid = 0.4436 ms) produced a raw Wilcoxon p = 0.0207 (uncorrected), but the FDR-adjusted p-value was 0.1037, above the 0.05 threshold — not statistically significant after correction. Other chest leads (V1--V5) produced t-test and Wilcoxon p-values $>0.05$ uncorrected, so none survived FDR correction. The physics-aware variant was effectively identical to the baseline in our experiments (pairwise mean differences $\approx 0$ across all leads), which is expected because limb leads are deterministically derived and provide no additional information for chest lead reconstruction.

\begin{table}[h]
\caption{Example Pairwise Comparison (Baseline vs Hybrid) — QRS Duration Error (ms)}
\label{tab:pairwise_example}
\begin{tabular}{lccc}
	oprule
Lead & Mean Diff (base - other) & Wilcoxon p (raw) & Wilcoxon p (FDR) \\
\midrule
...
Despite having 2.4$\times$ more parameters (40.8M vs 17.1M), the lead-specific architecture performed worse on 4 of 5 chest leads. See Table\textcolor{blue}{~\ref{tab:ablation_detail}}. We calculated a large effect size (Cohen's $d = 0.92$, 95\% CI [0.006, 0.072]) favoring the shared decoder, though paired tests across only 5 chest leads are low-powered; effect-size estimates were therefore used as the primary evidence for practical differences.
...
\subsection{Clinical Feature Preservation}
Following the ECGGenEval framework~\cite{ref63}, we evaluate preservation of clinically significant ECG features. Table~\ref{tab:clinical_features} summarizes the feature-level metrics computed on the test set (Lead II, 1,932 patients).

\begin{table}[h]
\caption{Clinical Feature Preservation (Feature-Level Evaluation)}
\label{tab:clinical_features}
\begin{tabular}{lcccc}
\toprule
\textbf{Feature} & \textbf{Ground Truth} & \textbf{Reconstructed} \\
\midrule

QRS Duration (ms) & 98.5 $\pm$ 12.3 & 102.1 $\pm$ 14.7 \\
PR Interval (ms) & 162.4 $\pm$ 22.1 & 158.9 $\pm$ 24.8 \\
QT Interval (ms) & 398.2 $\pm$ 32.6 & 385.4 $\pm$ 38.2 \\
Heart Rate (bpm) & 71.8 $\pm$ 14.2 & 72.3 $\pm$ 14.8  \\
\midrule
P-wave Amp. Corr. & \multicolumn{2}{c}{$r = 0.78$}   \\
T-wave Amp. Corr. & \multicolumn{2}{c}{$r = 0.84$}  \\
\bottomrule
\end{tabular}
\end{table}
