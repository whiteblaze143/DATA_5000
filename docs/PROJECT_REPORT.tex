...
\usepackage{ragged2e}
\usepackage{array}
\usepackage{siunitx} % improved numeric alignment
\sisetup{round-mode=places, round-precision=3, group-separator = {,}, detect-all}
\hypersetup{
    colorlinks=true,
    linkcolor=blue,
    citecolor=teal,
...
\begin{itemize}
    ...
    \item Feature- and diagnostic-level evaluation indicating which downstream tasks can be reasonably supported by reconstructed leads and which require caution.
\end{itemize}
...
\begin{tabular}{l S[table-format=1.4] S[table-format=1.5] S[table-format=2.2]}
\toprule
Variant & \multicolumn{1}{c}{$r$} & \multicolumn{1}{c}{MAE (mV)} & \multicolumn{1}{c}{SNR (dB)} \\ \midrule
...
\begin{tabular}{l S[table-format=1.3] S[table-format=1.3] S[table-format=2.2] l}
\toprule
Lead & \multicolumn{1}{c}{$r$} & \multicolumn{1}{c}{MAE (mV)} & \multicolumn{1}{c}{SNR (dB)} & Rank \\ \midrule
...
\toprule
Lead & \multicolumn{1}{c}{Shared $r$} & \multicolumn{1}{c}{Lead-spec $r$} & Better \\ \midrule
...
\toprule
Feature & \multicolumn{1}{c}{MAE} & Threshold \\ \midrule
QRS duration & 6.8 & 10 ms \\
PR interval & 9.3 & 20 ms \\
QT interval & 18.7 & 30 ms \\
Heart rate & 1.9 & 5 bpm \\ \midrule
...
\end{subfigure}
\caption{Clinical feature evaluation: (a) QRS duration Bland--Altman, (b) QT interval Bland--Altman, (c) mean absolute error (MAE) heatmap.}
...
\centering
\small
\begingroup\setlength{\extrarowheight}{2pt}\renewcommand{\arraystretch}{1.08}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l S[table-format=1.2] S[table-format=1.3] p{0.48\textwidth}}
\toprule
Lead & \multicolumn{1}{c}{\textbf{Corr(V4)}} & \multicolumn{1}{c}{\textbf{Recon $r$}} & \textbf{Comment} \\ \midrule
