...
Both architectures were trained on identical data with matched hyperparameters to ensure fair comparison:

\begin{enumerate}
    \item \textbf{Dataset:} PTB-XL with patient-wise splits (14,363 train / 1,914 val / 1,932 test)
    \item \textbf{Preprocessing:} Z-score normalization per lead, percentile outlier removal
    \item \textbf{Training:} 150 epochs maximum, early stopping (patience=20), AdamW optimizer
    \item \textbf{Learning rate:} $3 \times 10^{-4}$ (validated via sweep on full dataset)
    \item \textbf{Batch size:} 64
    \item \textbf{Random seed:} 42 (fixed for reproducibility)
    \item \textbf{Loss function:} MSE on chest leads only
\end{enumerate}

\textbf{Evaluation Protocol:}
After training, both models were evaluated on the held-out test set using a dedicated comparison script (\texttt{scripts/compare\_models.py}):

\begin{enumerate}
    \item Load trained weights: \texttt{best\_model.pt} from each model directory
    \item Run inference on full test set (1,932 patients)
    \item Reconstruct 12-lead ECG using physics module for limb leads
    \item Compute per-lead metrics: Pearson correlation, MAE, SNR
    \item Export results to \texttt{model\_comparison\_metrics.json}
    \item Generate side-by-side reconstruction visualizations
\end{enumerate}

\textbf{Statistical Analysis:}
We used the \texttt{scripts/compare\_variants.py} script to perform rigorous statistical tests:
\begin{itemize}
    \item Paired $t$-test and Wilcoxon signed-rank (non-parametric alternative)
    \item Cohen's $d$ effect size computation
